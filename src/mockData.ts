import type { FieldDefinition, MetricBinding, MetricDefinition, OutputCellConfig, TemplateConfig } from './types';
...
export const fieldCatalog: FieldDefinition[] = [
  {
    ...
    category: '资产负债表',
    unit: '亿元',
    description: '报告期末资产合计，常用于主体财务摘要和授信分析。',
    aliases: ['资产总额', 'totalAssets', '总资产(亿元)'],
  },
  {
    code: 'BS036',
    label: '总负债',
    category: '资产负债表',
    unit: '亿元',
    description: '报告期末负债合计。',
    aliases: ['负债总额', 'totalLiabilities', '总负债(亿元)'],
  },
  {
    code: 'IS010',
    label: '营业收入',
  ...
  },
  {
    ...
    category: '利润表',
    unit: '亿元',
    description: '报告期归属或合并口径净利润。',
    aliases: ['净利', 'netProfit', '净利润(亿元)'],
  },
  {
    code: 'CF020',
    label: '经营现金流',
    ...
    aliases: ['经营性现金流', '经营现金流净额', 'operatingCashFlow'],
  },
  {
    code: 'RR041',
    label: '资产负债率',
  ...
  },
  {
    ...
    category: '债券存续',
    unit: '亿元',
    description: '债券当前存续余额。',
    aliases: ['债券余额', '剩余本金'],
  },
  {
    code: 'BOND_REMAINING_TERM',
    label: '剩余期限',
    category: '债券存续',
    unit: '年',
    description: '距离到期日或行权日的剩余期限。',
    aliases: ['待偿期', 'remainingTerm'],
  },
  {
    code: 'BOND_RATING',
    label: '债项评级',
  ...
  },
  {
    ...
    category: '评级',
    unit: '-',
    description: '发行人或主体最新主体评级。',
    aliases: ['发行人评级', '主体信用评级'],
  },
  {
    code: 'BOND_YIELD',
    label: '收益率',
    category: '估值',
    unit: '%',
    description: '指定估值源下的收益率。',
    aliases: ['估值收益率', 'yield'],
  },
  {
    code: 'BOND_CLEAN_PRICE',
    label: '净价',
    category: '估值',
    unit: '元',
    description: '指定估值源下的估值净价。',
    aliases: ['中债估值净价', 'cleanPrice'],
  },
];
...
export function findFieldCandidates(query: string) {
  const normalized = query.trim().toLowerCase();
  if (!normalized) return fieldCatalog.slice(0, 6);

  return fieldCatalog
    .filter((field) => {
      const haystack = [field.label, field.code, field.category, ...field.aliases].join(' ').toLowerCase();
      return haystack.includes(normalized);
    })
    .slice(0, 8);
}

export function mockFetchMetricValue(metricCode: string, columnParam: string) {
  const numericYear = Number(columnParam || 0);
  const seed = metricCode.split('').reduce((sum, char) => sum + char.charCodeAt(0), 0);
  if (!numericYear && metricCode.startsWith('DM_')) return mockTemplateFieldValue(metricCode, columnParam);
  if (!numericYear && fieldCatalog.some((field) => field.code === columnParam)) return mockTemplateFieldValue(metricCode, columnParam);
...
export function buildInitialOutputConfigs(columnParams: string[], bindings: MetricBinding[]): Record<string, OutputCellConfig> {
  const configs: Record<string, OutputCellConfig> = {};
  bindings.forEach((binding) => {
    columnParams.forEach((columnParam, index) => {
      ...
      configs[targetCell] = {
        ...
        columnParam,
        status: 'ready',
        value: baseRows[binding.row - 1]?.[columnIndex - 1] ?? '',
        lastRefreshAt: '2026-05-07 15:46',
      };
    });
  });
  return configs;
